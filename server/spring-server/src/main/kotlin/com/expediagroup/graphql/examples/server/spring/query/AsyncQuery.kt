...
class AsyncQuery : Query {
    ...
    fun delayedEchoUsingCompletableFuture(
        msg: String,
    ...
    fun delayedEchoUsingReactorMono(
        msg: String,
        delayMilliseconds: Int,
    ): Mono<String> = Mono.just(msg).delayElement(Duration.ofMillis(delayMilliseconds.toLong()))

    @GraphQLDescription("Delays for given amount of time using Coroutine and then echoes the string back.")
    suspend fun delayedEchoUsingCoroutine(
        msg: String,
        delayMilliseconds: Int,
    ): String =
        coroutineScope {
