...
class SimpleQueries(
    ...
    fun helloWorld(
        @GraphQLDescription("optional name, defaults to `World` if not specified") name: String? = null,
    ) = "Hello ${name ?: "World"}"
    ...
    fun complexObjectQuery(): ComplexObject =
        ComplexObject(
            id = random.nextInt(),
            name = "whatever",
            details =
    ...
    fun nestedObjectQuery(): NestedObject =
        NestedObject(
            id = random.nextInt(),
            name = "nested",
            children =
    ...
    fun interfaceQuery(): BasicInterface =
